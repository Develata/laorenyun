...
2. MediaRecorder 保留原始字节、真实 MIME。停止后先存浏览器IndexedDB，再整段二进制上传Host；收到文件/数据库持久回执才显示“录音已保存”。异常停止保留收到的有限字节并标不完整，不能伪称可完整解码；录制中尚非实时归档，见ADR-0015。
3. Host 从原件生成 16kHz/16-bit/mono WAV 识别副本；腾讯 ASR 极速文件识别同步请求（由 Host 后台任务执行）。所有身份/密钥在 Host。
4. 保存raw ASR文字、可用中立分段与requestId/engine/latency；当前session/source仍匹配才通过原生input action写入。期间用户已有文字时保留旧文字，明确选择追加/替换，不静默覆盖。
...
7. TTS 只读本次最终 assistant 可见文本，不朗读 tool trace/推理/错误堆栈；播放被浏览器拒绝时显示“点一下播放”。可停止、重播；独立“关闭自动朗读”偏好留后续。
...
持久原件与云操作解耦。ASR 失败：“这段录音已经保存，但文字识别失败。”提供重试和文字输入；存储失败则必须写“尚未保存”，保留浏览器副本并提供重试，不能显示前一种文案。未取得完整持久回执前关闭标签页有丢失未上传尾部的风险，要明确提示。

取消识别停止本地网络；已传云数据/可能计费不可伪称撤回。Flash没有持久TaskId轮询恢复，响应丢失时保留原件并提示显式重新识别可能重复处理。TTS 失败不阻塞下一轮采访。重连不自动重播旧回答，不自动再提交文字。UI 忙闲由状态机决定。
