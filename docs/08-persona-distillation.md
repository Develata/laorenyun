...
仅可选快照作为自传renderer的数据风格槽；不改变采访system prompt、工具或权限。历史 Phase 4 renderer 曾只采用有引文支持的有限转场、保持完整事实原句；该历史限制不适用于当前 Writer。[Phase 4](phase-4.md)列出真实模型观察和未证实项。

## v0.2 展示与使用

结果“我的表达方式”位于“我的自传”；有书稿时默认折叠，不阻挡阅读。用常用表达/叙事习惯/句子与节奏/称呼/情绪表达呈现，并可展开真实引文。成功创建后默认用于下一次自传，用户可关闭。新 Writer 可自由调整语言；Persona 仅传观察说明，事实性引用示例不混入事实宇宙。源不足只列 unknown，不生成空引用观察。历史 Phase4 renderer 的有限转场限制不再约束 narrative-v2。
