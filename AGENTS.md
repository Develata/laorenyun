...
始终中文交流。当前只完成 Phase 0；未经后续实现任务授权，不创建生产 runtime、UI、数据库实现或最终镜像。
...
有 `.codegraph/` 时，理解或定位代码先用 CodeGraph；Markdown/字符串用 rg。变更后执行匹配验证、检查 diff；最终写 `验证：...`，分别报告文档变更、代码实现、运行和发布。Phase 0 不用虚构 build/test 通过。
