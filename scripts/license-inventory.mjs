...
function walk(root){for(const e of readdirSync(root,{withFileTypes:true})){if(e.isSymbolicLink())continue;const p=root+'/'+e.name;if(e.isDirectory())walk(p);else if(e.name==='package.json'){let x;try{x=JSON.parse(readFileSync(p,'utf8'));}catch{continue;}if(x.name&&x.version)packages.set(x.name+'@'+x.version,{name:x.name,version:x.version,license:x.license??null,source:typeof x.repository==='string'?x.repository:x.repository?.url??x.homepage??null,role:'DSH runtime',licenseFiles:readdirSync(root).filter(n=>/^(license|licence|copying|notice)/i.test(n))});}}}
