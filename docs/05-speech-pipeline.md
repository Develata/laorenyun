...
通过SDK总览发现Flash，适合录完后识别，省掉异步轮询/公网回调/COS。课程/演示默认引擎为已匹配普通免费包实测的 `16k_zh`；`16k_zh_en` 大模型1.0仍可显式选择，额度独立；Flash参数页并未列出 `16k_zh_en_2.0`，不能沿用V2/异步接口的引擎名。质量/费用/速度综合选择见 [ADR-0006](adr/0006-tencent-cloud-speech.md)。目标口音尚未给出，普通引擎账号调用已验证，真人录音对比仍须验收。
