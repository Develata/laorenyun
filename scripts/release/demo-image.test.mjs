...
test('private demo requires a full commit and a private exact package',()=>{
 ...
 privateVisibility(404,{},true);privateVisibility(200,{name:'laorenyun-demo-private',visibility:'private'});
...
test('private workflow is manual, does not publish image artifacts or change source releases',()=>{
 ...
 const p=readFileSync(new URL('../demo-image/publish.mjs',import.meta.url),'utf8');assert.ok(!p.includes("'release'"));assert.ok(!p.includes('docker build'));assert.ok(p.includes("assert.equal(existing,false"));
